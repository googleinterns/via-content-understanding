...
class PROBABILITY_HOLDER:
	...
	def sorted_append(self, class_index, video_index, probability):
		...
		"""
		i = self.binary_search(self, candidate_probs, probability)
		
		self.candidate_probs[class_index] = candidate_probs[:i] + [candidate_probs] + candidate_probs[i:]
		self.candidates[class_index] = self.candidates[class_index][:i] + [video_index] + self.candidates[class_index][i:]

	def sorted_insert(self, class_index, video_index, probability):
	...
	def add_data(self, video_index, output_probs):
		...
		for class_index in range(self.num_classes):
			...
			if len(self.candidates[class_index]) < self.k:
				self.sorted_append(class_index, video_index, probability)
			elif output_probs[true_class_index] > self.candidates[class_index][0]:
				self.sorted_insert(class_index, video_index, probability)
