...
class TestLoss(unittest.TestCase):
  ...
  def test_correct(self):
    ...
    y_predicted = tf.convert_to_tensor([[1,0],[0,1], [1,1]])

    loss_out = loss.custom_crossentropy(y_actual, y_predicted)
    self.assertEqual(loss_out, tf.zeros((1,)))

  def test_incorrect(self):
    y_actual = tf.convert_to_tensor([[1,0]])
    y_predicted = tf.convert_to_tensor([[0.5,0]])

    loss_out = loss.custom_crossentropy(y_actual, y_predicted, epsilon=0)
  ...
  def test_alpha(self):
    ...
    loss_out = loss.custom_crossentropy(y_actual, y_predicted, epsilon=0, alpha=0.7)
    self.assertEqual(loss_out, tf.convert_to_tensor([-1*0.7*tf.math.log(0.5)]))

  def test_epsilon(self):
    y_actual = tf.convert_to_tensor([[1,0],[0,1], [1,1]])
    y_predicted = tf.convert_to_tensor([[0,1],[1,0], [0,0]])
